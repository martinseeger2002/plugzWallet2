...
const networksPath = '../static/js/networks.js';
const networksContent = fs.readFileSync(networksPath, 'utf8');
const networkMatch = networksContent.match(/export const coins = (\[[\s\S]*?\]);/);
const coins = networkMatch ? eval(networkMatch[1]) : [];

// Get coin config based on ticker
function getCoinConfig(ticker) {
